...
export function Noise3D(position:THREE.Vector3) {
  ...
  const a = new THREE.Vector3(Math.floor(p.x), Math.floor(p.y), Math.floor(p.z));
  ...
  const k2 = perm(new THREE.Vector4(k1.x+b.z, k1.y+b.z, k1.x+b.w, k1.y+b.w));
  const c = new THREE.Vector4(k2.x+a.z, k2.y+a.z, k2.z+a.z, k2.w+a.z);
  const k3 = perm(c);
  const k4 = perm(new THREE.Vector4(c.x+1.0, c.y+1.0, c.z+1.0, c.w+1.0));
  const o1 = new THREE.Vector4(fract(k3.x*(1.0/41.0)), fract(k3.y*(1.0/41.0)), fract(k3.z*(1.0/41.0)), fract(k3.w*(1.0/41.0)));
  const o2 = new THREE.Vector4(fract(k4.x*(1.0/41.0)), fract(k4.y*(1.0/41.0)), fract(k4.z*(1.0/41.0)), fract(k4.w*(1.0/41.0)));
  const o3 = new THREE.Vector4(o2.x*d.z+o1.x*(1.0-d.z), o2.y*d.z+o1.y*(1.0-d.z), o2.z*d.z+o1.z*(1.0-d.z), o2.w*d.z+o1.w*(1.0-d.z));
  const o4 = new THREE.Vector2(o3.y*d.x+o3.x*(1.0-d.x), o3.w*d.x+o3.z*(1.0-d.x));
